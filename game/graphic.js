function init()
{
    ...
    for (const position of ground) {
        const [x, y] = position;
        const distance = Math.sqrt((x - 50) ** 2 + (y - 0) ** 2);
        if (distance < distanceMin) {
            newPosition = position;
            distanceMin = distance;
        }
    }
    player1 = new Player("player1", 0xffff00, new THREE.Vector2(newPosition[0], newPosition[1]), 0);
...
{
    pointLight = new THREE.PointLight(color, 5000, 5500);
